...
function Shell() {
  ...
  const { applyRecurrences } = useStore()

  // apply missed recurrences on load
  useEffect(() => { applyRecurrences?.() }, [applyRecurrences])
  ...
  return (
    <div className="min-h-screen text-neutral-900 dark:text-neutral-100">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        {/* Show cross-app overview ONLY on Net Worth */}
        {tab === 'Net Worth' && (
          <>
            <Insights />
            {/* Removed <Overview /> to eliminate the duplicate row */}
            <Reminders />
            <NetWorth />
          </>
        )}
...
/* --- Keep these, rendered only on Net Worth --- */
function Reminders() {
  const { data } = useStore()
  const items = []
    ...
    <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
      <div className="card">
        ...
        <div className="mt-1 text-sm">
          {highestAPR
            ? <>Target <span className="font-semibold">{highestAPR.name}</span> next ({highestAPR.apr}% APR)</>
            : 'Add a debt to get suggestions'}
        </div>
      </div>
      <div className="card">
        ...
        <div className="mt-1 text-sm">
          {nextDue
            ? <>{nextDue.type}: <span className="font-semibold">{nextDue.name}</span> on {nextDue.date.toLocaleDateString()}</>
            : 'No scheduled items'}
        </div>
      </div>
      <div className="card">
        ...
        {achievements.length
          ? achievements.map(a => <div key={a.id} className="text-sm">🏆 {a.text}</div>)
